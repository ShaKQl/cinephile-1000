<template>
    <section class="info" :class="{active: open}">
        <Transition name="infoblock">
            <div class="info__wrapper" v-if="current">
                ...
                <div class="info__content">
                    ...
                    <p class="info__about">
                        {{ new Date(current.release_date || current.first_air_date).getFullYear() }}, {{ current.genres.map((val) => val.name).join(', ')
                        }}
                        <span class="info__time" v-if="current.runtime">
                            {{ new Date(0, 0, 0, 0, current.runtime).getHours() }}h {{ new
                                Date(0, 0, 0, 0, current.runtime).getMinutes() }}m
                        </span>
        ...
        <font-awesome-icon class="info__close" :icon="['fas', 'xmark']" @click="$emit('close')"/>
    </section>
</template>
